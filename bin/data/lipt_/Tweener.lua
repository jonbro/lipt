...
function Tweener.equations.outSine(t, b, c, d) return c * math.sin(t / d * (PI / 2)) + b end
...
function Tweener:update()
  for i=table.maxn(self.runningTweens),1,-1 do
    ...
    if tt.update then tt.update() end
    ...
    if tt.currentTime > tt.time then
      ...
      -- set the tween to nil so that it is no longer in memory and can be swept
